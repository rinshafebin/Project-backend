...
User = get_user_model()
otp_storage = {}  # simple in-memory OTP; for real use store in DB/Redis
...
class ClientRegisterView(APIView):
    ...
    def post(self, request):
        ...
        serializer.is_valid(raise_exception=True)
        user = serializer.save()
        # local task - welcome email
        send_welcome_email.delay(user.email, user.username)
        # emit event to client-service to create ClientProfile
...
class AdvocateRegisterView(APIView):
    ...
    def post(self, request):
        ...
        serializer.is_valid(raise_exception=True)
        user = serializer.save()
        # profile data collected inside serializer (attached as attribute)
...
class LoginView(APIView):
    ...
    def post(self, request):
        ...
        if user.role in ["admin", "advocate"] and user.mfa_enabled:
            return Response({"message": "MFA required", "mfa_type": user.mfa_type, "user_id": user.id})
...
class ForgetPasswordView(APIView):
    ...
    def post(self, request):
        ...
        otp = str(random.randint(100000, 999999))
        otp_storage[email] = otp
        # send by email (sync here; could make an async task)
        send_mail(
            subject="Your password reset OTP",
...
# Token validation endpoint for other services to reuse user identity
class ValidateTokenView(APIView):
    permission_classes = [IsAuthenticated]
    def get(self, request):
        user = request.user
