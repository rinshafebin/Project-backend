...
class AdvocateRegisterSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = User
        fields = ("username", "email", "password", "confirm_password", "role", "phone_number", "bar_council_number")

    def validate(self, data):
        if data["password"] != data["confirm_password"]:
            raise serializers.ValidationError({"password": "Password fields didn't match"})
        if data.get("role") != "advocate":
            raise serializers.ValidationError({"role": "Role must be advocate"})
        return data
    ...
    def create(self, validated_data):
        ...
        bar_no = validated_data.pop("bar_council_number")
        user = User.objects.create_user(
            username=validated_data["username"],
            email=validated_data["email"],
            password=validated_data["password"],
        ...
        user._initial_profile_data = {"phone_number": phone, "bar_council_number": bar_no}
        return user
...
class ResetPasswordSerializer(serializers.Serializer):
    ...
    def validate_otp(self, value):
        if not value.isdigit() or len(value) != 6:
            raise serializers.ValidationError("OTP must be 6 numeric digits")
        return value
